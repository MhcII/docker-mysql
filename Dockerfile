...
ENV MYSQL_MAJOR 5.0
ENV MYSQL_VERSION 5.0.27

RUN apt-get update && apt-get install -y ca-certificates wget --no-install-recommends && rm -rf /var/lib/apt/lists/* \
	&& wget "https://downloads.mysql.com/archives/get/file/mysql-$MYSQL_VERSION.tar.gz" -O mysql.tar.gz \
	&& wget "https://downloads.mysql.com/archives/gpg/?file=mysql-$MYSQL_VERSION.tar.gz" -O mysql.tar.gz.asc \
	&& apt-get purge -y --auto-remove ca-certificates wget \
	&& export GNUPGHOME="$(mktemp -d)" \
